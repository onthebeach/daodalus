    ...
    describe Where do
      ...
      describe "#eq" do
        it 'adds an equal clause to the where' do
          query.eq(5).criteria.should eq ({'cats' => 5})
        end
        it 'can also be called using an alias' do
          query.equals(5).criteria.should eq ({'cats' => 5})
        end
      end

      describe "#ne" do
        it 'adds a not equal clause to the where' do
          query.ne(5).criteria.should eq ({'cats' => { '$ne' => 5 }})
        end
        it 'can also be called using an alias' do
          query.not_equal(5).criteria.should eq ({'cats' => { '$ne' => 5 }})
        end
      end

      describe "#lt" do
        it 'adds a less than clause to the where' do
          query.lt(5).criteria.should eq ({'cats' => { '$lt' => 5 }})
        end
        it 'can also be called using an alias' do
          query.less_than(5).criteria.should eq ({'cats' => { '$lt' => 5 }})
        end
      end

      describe "#lte" do
        it 'adds a less than or equal clause to the where' do
          query.lte(5).criteria.should eq ({'cats' => { '$lte' => 5 }})
        end
        it 'can also be called using an alias' do
          query.less_than_or_equal(5).criteria.should eq ({'cats' => { '$lte' => 5 }})
        end
      end

      describe "#gt" do
        it 'adds a greater than clause to the where' do
          query.gt(5).criteria.should eq ({'cats' => { '$gt' => 5 }})
        end
        it 'can also be called using an alias' do
          query.greater_than(5).criteria.should eq ({'cats' => { '$gt' => 5 }})
        end
      end

      describe "#gte" do
        it 'adds a greater than or equal clause to the where' do
          query.gte(5).criteria.should eq ({'cats' => { '$gte' => 5 }})
        end
        it 'can also be called using an alias' do
          query.greater_than_or_equal(5).criteria.should eq ({'cats' => { '$gte' => 5 }})
        end
      end

      describe "#in" do
        it 'adds an in clause to the where' do
          query.in(5,6).criteria.should eq ({'cats' => { '$in' => [5,6] }})
        end
      end

      describe "#nin" do
        it 'adds a not in clause to the where' do
          query.nin(5,6).criteria.should eq ({'cats' => { '$nin' => [5,6] }})
        end
        it 'can also be called using an alias' do
          query.not_in(5,6).criteria.should eq ({'cats' => { '$nin' => [5,6] }})
        end
      end

      describe "#all" do
        it 'adds an all clause to the where' do
          query.all(5,6).criteria.should eq ({'cats' => { '$all' => [5,6] }})
        end
      end

      describe "#size" do
        it 'adds a size clause to the where' do
          query.size(5).criteria.should eq ({'cats' => { '$size' => 5 }})
        end
      end

      describe "#mod" do
        it 'adds a mod clause to the where' do
          query.mod(6,2).criteria.should eq ({'cats' => { '$mod' => [6,2] }})
        end
        it 'can also be called using an alias' do
          query.modulus(6,2).criteria.should eq ({'cats' => { '$mod' => [6,2] }})
        end
      end

      describe "#exists" do
        it 'adds an exists clause to the where' do
          query.exists.criteria.should eq ({'cats' => { '$exists' => true }})
        end
      end

      describe "#size" do
        it 'adds a not exists clause to the where' do
          query.does_not_exist.criteria.should eq ({'cats' => { '$exists' => false }})
        end
      end
